		   ...
		   int  cmd_show)
{
	...
	if(window)
	{
		...
		// @ font
		io_file file = io_readfile("../data/Fontin_48.font");
		if(file.source)
		{
		...
		while(running)
		{
		    ...
		    graphics_primitive_render_text(&primitive, "hello", sizeof("hello"), vec2(1.0, 1.0));
		    // imgui_beginframe(&imgui);
		    // imgui_label(&imgui, "label", 0.1, 0.1, 2, 1);
